...
def create_chart(user_1: dict, user_2: dict):
    ...
    ax.plot(time_1, [user_1['message_time'][i] for i in time_1],
            'o-', color='#0c0fea', label=user_1['name'])
    time_2 = sorted(user_2['message_time'])
    ax.plot(time_2, [user_2['message_time'][i] for i in time_2],
    ...
    plt.title(title)
    plt.xlabel('chat session duration')
    plt.ylabel('message timing')
    plt.legend()
...
class TestMessageReliability(MultipleDeviceTestCase):
    ...
    def test_message_reliability(self, messages_number, message_wait_time, connection_problem_wait_time):
        ...
        try:
            self.create_drivers(2, max_duration=10800)
            ...
            for i in range(int(messages_number / 2)):
                ...
                try:
                    receive_time = timeit(wrapper(device_2_chat.find_full_text, message_1, message_wait_time), number=1)
                    duration_time = round(time.time() - start_time, ndigits=2)
                    user_b_message_timing[duration_time] = receive_time
                ...
                try:
                    receive_time = timeit(wrapper(device_1_chat.find_full_text, message_2, message_wait_time), number=1)
                    duration_time = round(time.time() - start_time, ndigits=2)
                    user_a_message_timing[duration_time] = receive_time
